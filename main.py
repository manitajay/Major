...
def send_marks(subject_code):
    ...
    for scholar_no in range(1,400):
        
        print(scholar_no)

        ss='D'+str(scholar_no+1)
        cl=lf[ss]
        ml=cl.value
        print(ml)
        if(ml is None):
        ...
        subject= f"Results of {subject_code}"
        body= f"Dear scholar no {scholar_no}, you have scored {cl.value} marks out of {cl2.value} in {subject_code}."

        

        sm.send_mail(subject,body,ml)
    print("All results mail sent!")
...
def response(input_text,ask_login,login,user_name,subject_code,scholar_no,login_teacher):
    ...
    print(user_message)
    if(user_message in ("send result mails")):
        print("start....")
        if(login_teacher==False):
            return f"please login as a teacher",ask_login,login,user_name,subject_code,scholar_no,login_teacher
        else:
            ...
            print(subject_code)
            send_marks(subject_code)
            return f"All results mail sent",ask_login,login,user_name,subject_code,scholar_no,login_teacher
